...
import type { AnnotationMetadata, ParsedAnnotation } from "../types/annotations.js";
import type { Config } from "../types/config.js";
import { type EnhancedHistoryEntry,type HistoryEntry } from "../types/history.js";
import type {
    DataCompleteness,
    ExecutionOutcomes,
    PromptReviewData,
...
export class ReviewDataBuilder {
    ...
    constructor(private config: Config & { modules?: ModuleEntry[] }) {
        this.historyManager = new HistoryManager(
            config.historyDir || path.join(getDataDir(), "history"),
            config.annotationDir,
        );
    ...
    private async getAnnotations(): Promise<ParsedAnnotation[]> {
        ...
        try {
            ...
            const annotationFiles = files.filter(
                (f) =>
    ...
    private calculateUsageStatistics(entries: HistoryEntry[], annotations: ParsedAnnotation[]): UsageStatistics {
    ...
    private calculateDataCompleteness(prompts: PromptReviewData[], entries: HistoryEntry[]): DataCompleteness {
        if (entries.length === 0) {
            return {
                with_annotations: 0,
