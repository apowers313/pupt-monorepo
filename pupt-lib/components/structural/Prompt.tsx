import { z } from 'zod';
import { Component, findChildrenOfType, wrapWithDelimiter } from 'pupt-lib';
import { ROLE_PRESETS, DEFAULT_CONSTRAINTS, PROVIDER_ADAPTATIONS, STANDARD_GUARDRAILS } from '../presets';
import type { PuptNode, PuptElement, RenderContext, ComponentType } from 'pupt-lib';
...
const promptDefaultsSchema = z.union([
...
export class Prompt extends Component<PromptProps> {
  ...
  render(props: PromptProps, _resolvedValue: void, context: RenderContext): PuptNode {
    ...
    const constraintsContainers = findChildrenOfType(childArray, Constraints) as PuptElement[];
    const hasConstraintsContainer = constraintsContainers.length > 0;

    // Detect Phase 8 container components
    const guardrailsContainers = findChildrenOfType(childArray, Guardrails) as PuptElement[];
    ...
    if (hasConstraintsContainer) {
      ...
      const containerProps = container[PROPS] as Record<string, unknown>;
  ...
  private shouldIncludeSection(
    defaultsValue: boolean | undefined,
    configValue: boolean,
  ): boolean {
