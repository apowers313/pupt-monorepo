...
export class Tone extends Component<ToneProps> {
    ...
    render(props: ToneProps, _resolvedValue: undefined, _context: RenderContext): PuptNode {
        ...
        if (formality) {
            characteristics.push(`formality: ${formality}`);
        }
        if (energy) {
            characteristics.push(`energy: ${energy}`);
        }
        if (warmth) {
            characteristics.push(`warmth: ${warmth}`);
        }
